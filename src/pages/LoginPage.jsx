...
const LoginPage = () => {
    ...
    const handleKakaoLogin = () => {
        ...
        const REDIRECT_UTL = process.env.REACT_APP_KAKAO_REDIRECT_URL;

        console.log("API_KEY : ", API_KEY);
        console.log("REDIRECT_UTL : ", REDIRECT_UTL);

        if(!API_KEY || !REDIRECT_UTL) {
            alert("카카오 설정 오류 : 환경변수를 확인해주세요.");
            return;
        }

        const kakaoAuthUrl = `https://kauth.kakao.com/oauth/authorize?client_id=${API_KEY}&redirect_uri=${REDIRECT_UTL}&response_type=code`;
